...
// The shared secret stays the token's SHA-256 digest even on the SHA-512
// derivation: `x-ui grpc-connect` never keeps the raw token, so that digest is
// the only value both sides provably hold. Changing it would re-provision every
// node at once, which the derivation change deliberately does not require.
func TestSharedSecretStaysTheStoredDigest(t *testing.T) {
	sum := sha256.Sum256([]byte(token))
	if HashSecret(token) != hex.EncodeToString(sum[:]) {
		t.Fatal("HashSecret no longer matches what a node stores")
	}
}

...
// This file is a verbatim mirror of internal/tokenaead in the wingsv panel
// (v.wingsnet.org). Both ends of the management gRPC must derive identical
// keys, so any change here has to land in that copy too.
package tokenaead
...
// an operator sees which panels are still on the old one, which is the only way

...
func(p *Process)Start()error{
  ...
  config:=filepath.Join(p.dir,"psiphond.config")
  if _,err:=os.Stat(config);os.IsNotExist(err){
    // Generate the complete private server configuration. server-entry.dat is
    // client-facing metadata and cannot be used instead of psiphond.config.

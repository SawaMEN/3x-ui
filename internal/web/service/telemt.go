...
func ensureTelemtConfig() error {
	...
	if err := os.WriteFile(telemtConfigPath, []byte(data), 0o600); err != nil {
...
func (TelemtService) GetConfig() (TelemtConfig, error) {
	...
	if err := toml.Unmarshal(b, &raw); err != nil {
		return TelemtConfig{}, fmt.Errorf("telemt: parse config: %w", err)
	}
	...
	if c.Enabled && systemctl("is-active", "--quiet", telemtServiceName) == nil {
		if live, err := fetchTelemtRuntimeConfig(); err == nil {
			if live.General.FastMode != nil {
				c.FastMode = *live.General.FastMode
			}
			if live.General.Modes.Classic != nil {
				c.Classic = *live.General.Modes.Classic
			}
			if live.General.Modes.Secure != nil {
				c.Secure = *live.General.Modes.Secure
			}
			if live.General.Modes.TLS != nil {
				c.TLS = *live.General.Modes.TLS
			}
			if live.Censorship.TLSDomain != "" {
				c.SNI = live.Censorship.TLSDomain
			}
		}
	}
	return c, nil
...
func fetchTelemtRuntimeConfig() (telemtRuntimeConfigResponse, error) {
	...
	if !envelope.OK {
		return telemtRuntimeConfigResponse{}, errors.New("telemt: runtime config API rejected request")
	}
...
func (TelemtService) SaveConfig(c TelemtConfig) error {
	...
	if wasActive {
		if err := systemctl("restart", telemtServiceName); err != nil {
			if oldErr == nil {
				_ = os.WriteFile(telemtConfigPath, old, 0o600)
...
func (TelemtService) ListProxies() ([]TelemtProxy, error) {
	...
	if err := toml.Unmarshal(b, &raw); err != nil {
		return nil, fmt.Errorf("telemt: parse config: %w", err)
	}
...
func ensureTelemtMekoFixInstalled() error {
	...
	if _, err := os.Stat("/usr/local/x-ui/telemt-meko-fix.sh"); errors.Is(err, os.ErrNotExist) {
		if err := download(scriptURL, "/usr/local/x-ui/telemt-meko-fix.sh", 0o700); err != nil {
	...
	if _, err := os.Stat("/etc/systemd/system/telemt-meko-fix.service"); errors.Is(err, os.ErrNotExist) {
		if err := download(unitURL, "/etc/systemd/system/telemt-meko-fix.service", 0o644); err != nil {

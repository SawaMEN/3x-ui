...
import (
	...
	wingsvproto "github.com/SawaMEN/3x-ui/v3/internal/wingsv/proto"
...
func (s *InboundService) AddVKTurnProxyClient(data *model.Inbound) (bool, error) {
	...
	existEmail, err := s.clientService.checkEmailsExistForClients(s, checkEmails)
	if err != nil {
		return false, err
	}
	if existEmail != "" {
...
func (s *InboundService) UpdateVKTurnProxyClient(data *model.Inbound, clientID string) (bool, error) {
	...
	if !strings.EqualFold(client.Email, previous.Email) {
		existEmail, err := s.clientService.checkEmailsExistForClients(s, []model.Client{{ID: client.ID, Email: client.Email}})
		if err != nil {
			return false, err
		}
		if existEmail != "" {

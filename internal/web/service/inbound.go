...
func (s *InboundService) GetClientsForInboundsBySubId(inboundIDs []int, subId string) (map[int][]model.Client, error) {
	return s.clientService.ListForInboundsBySubId(nil, inboundIDs, subId)
}
...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	if isVKTurnProxyProtocol(inbound.Protocol) {
		if _, vkErr := s.applyVKTurnProxyInboundOnSave(nil, inbound); vkErr != nil {
...
func (s *InboundService) delInbound(id int) (bool, func(), error) {
	...
	if loadErr == nil && ib.Protocol == model.VKTurnProxy {
		s.cleanupVKTurnProxyInboundPeers(&ib)
	}
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	vkNeedRestart := false
	if isVKTurnProxyProtocol(inbound.Protocol) {
		restart, vkErr := s.applyVKTurnProxyInboundOnSave(oldInbound, inbound)
		if vkErr != nil {
			return inbound, false, vkErr
		}
	...
	needRestart = needRestart || vkNeedRestart

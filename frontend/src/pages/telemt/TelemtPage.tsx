...
export default function TelemtPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try { await refreshInFlight.current; } finally { refreshInFlight.current = null; }
  }, [form]);
        ...
        <Layout className="content-shell">
          <Layout.Content className="content-area">
            <div className="telemt-shell">
              ...
              {!status.installed && <Alert type="warning" showIcon message="Бинарник Telemt не установлен" description="Установите актуальную сборку проекта — бинарник и systemd-служба будут установлены автоматически." />}
              ...
              <Card title={<Space><ApiOutlined /> Управление прокси</Space>} className="telemt-card">
                <Form form={createForm} layout="vertical" onFinish={createProxy}>
                  <Form.Item name="name" label="Название прокси" rules={[{ required: true, message: 'Введите название' }]}>
                    <Input placeholder="Telegram Proxy" />
                  </Form.Item>
                  <Typography.Text type="secondary">Публичный адрес определяется автоматически по домену или IP, с которого открыта панель.</Typography.Text>
                ...
                {proxies.length > 0 && <div className="telemt-proxy-list">{proxies.map((item) => <div className="telemt-proxy-item" key={item.name}><div><Typography.Text strong>{item.name}</Typography.Text><Typography.Paragraph copyable={{ text: item.link }} ellipsis={{ rows: 1 }} code style={{ margin: '4px 0 0' }}>{item.link}</Typography.Paragraph></div><Space wrap><Tag>{item.host}:{item.port}</Tag><Button htmlType="button" icon={<CopyOutlined />} onClick={() => navigator.clipboard.writeText(item.link)}>Копировать</Button><Button htmlType="button" danger icon={<DeleteOutlined />} loading={loading} onClick={() => void deleteProxy(item.name)}>Удалить</Button></Space></div>)}</div>}
              </Card>
              <Card title={<Space><SafetyCertificateOutlined /> MEKO V3 fix</Space>} className="telemt-card">
                <Alert type="info" showIcon message="Фикс by MEKO для Telemt" description="V3 использует u32 fingerprint для распознавания iOS-клиентов и ограничивает новые TCP SYN для остальных клиентов. Порт берётся автоматически из настройки Telemt." />
                <Space wrap style={{ marginTop: 16 }}><Tag color={status.mekoEnabled ? 'green' : 'default'}>{status.mekoEnabled ? 'MEKO включён' : 'MEKO выключен'}</Tag><Button htmlType="button" type={status.mekoEnabled ? 'default' : 'primary'} icon={<SafetyCertificateOutlined />} onClick={() => action(status.mekoEnabled ? 'meko-disable' : 'meko-enable')} disabled={!status.installed || !status.active} loading={loading}>{status.mekoEnabled ? 'Выключить MEKO V3' : 'Включить MEKO V3'}</Button></Space>
                <Typography.Paragraph type="secondary" style={{ marginTop: 12, marginBottom: 0 }}>Параметры V3 фиксированы в системном сервисе: iOS fingerprint — без лимита, остальные клиенты — до 54 новых SYN в минуту с burst 1, затем TCP reset.</Typography.Paragraph>
              </Card>
              <Card title={<Space><SettingOutlined /> Конфигурация Telemt</Space>} className="telemt-card">
                <Form form={form} layout="vertical" onFinish={save} initialValues={defaults}>
                  <Row gutter={16}><Col xs={24} md={8}><Form.Item name="port" label="Порт" rules={[{ required: true }, { type: 'number', min: 1, max: 65535 }]}><InputNumber style={{ width: '100%' }} /></Form.Item></Col><Col xs={24} md={16}><Form.Item label="Секрет" required><Space.Compact block><Form.Item name="secret" noStyle rules={[{ required: true }, { pattern: /^[0-9a-fA-F]{32}$/, message: 'Нужно ровно 32 hex-символа' }]}><Input placeholder="0123456789abcdef0123456789abcdef" /></Form.Item><Button htmlType="button" icon={<SafetyCertificateOutlined />} onClick={generateSecret}>gen</Button></Space.Compact></Form.Item></Col></Row>
                  <Row gutter={16}><Col xs={24} md={12}><Form.Item name="sni" label="SNI" rules={[{ required: true }]}><Input placeholder="petrovich.ru" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name="tls" label="Fake-TLS (ee)" valuePropName="checked"><Switch checkedChildren="Включён" unCheckedChildren="Выключен" /></Form.Item></Col></Row>
                  <Row gutter={16}><Col xs={24} md={8}><Form.Item name="ipv4" label="IPv4" valuePropName="checked"><Switch /></Form.Item></Col><Col xs={24} md={8}><Form.Item name="ipv6" label="IPv6" valuePropName="checked"><Switch /></Form.Item></Col><Col xs={24} md={8}><Form.Item name="fastMode" label="Fast mode" valuePropName="checked"><Switch /></Form.Item></Col></Row>
                  <Button type="primary" htmlType="submit" loading={loading}>Сохранить конфигурацию</Button>
                </Form>
              </Card>
            </div>
          </Layout.Content>

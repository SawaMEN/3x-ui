...
interface VkTurnProxyLogModalProps {
  open: boolean;
  onClose: () => void;
}

// Logs viewer for the vk-turn-proxy standalone relay listener. Modeled on
// LogModal (the xray panel logs viewer) but hits the dedicated server
// endpoint that streams the relay's own log lines.
export default function VkTurnProxyLogModal({ open, onClose }: VkTurnProxyLogModalProps) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const msg = await HttpUtil.post<string[]>(`/panel/api/server/vk-turn-proxy/logs/${rows}`, {
        level,
      });
      if (msg?.success) {
        setLogs(msg.obj || []);

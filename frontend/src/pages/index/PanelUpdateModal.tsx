...
export default function PanelUpdateModal({
  ...
  async function runUpdate() {
    ...
    const statusMsg =
      outcome === 'failed'
        ? await HttpUtil.get<PanelUpdateStatus>('/panel/api/server/getUpdateStatus', undefined, {
            silent: true,
            timeout: 2000,
          }).catch(() => undefined)
        : undefined;
    const failedStatus = statusMsg?.obj;
    modal[outcome === 'failed' ? 'error' : 'warning']({
      ...
      content: (
        <div>
          ...
          {outcome === 'failed' && failedStatus?.message && (
            <div style={{ marginTop: 12 }}>
              <strong>{t('pages.index.panelUpdateReason')}</strong> {failedStatus.message}
            </div>
          )}
          {failedStatus?.logFile && (
            <div style={{ marginTop: 8 }}>
              <strong>{t('pages.index.panelUpdateLog')}</strong> <code>{failedStatus.logFile}</code>
            </div>
          )}
        </div>
      ),
